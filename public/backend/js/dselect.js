    ...
    if (found.length < 1) {
        ...
        if (creatable) {
            ...
            if (event.key === "Enter") {
                ...
                const toggler =
                    target.nextElementSibling.getElementsByClassName(
                        classToggler
                    )[0];
...
function dselect(el, option = {}) {
    ...
    function itemTags(options) {
        ...
        for (const option of options) {
            if (option.tagName === "OPTGROUP") {
                items.push(
                    `<h6 class="dropdown-header">${option.getAttribute(
                        "label"
                    )}</h6>`
                );
            } else {
                ...
                const disabled =
                    el.multiple && option.selected ? " disabled" : "";
    ...
    <div class="dropdown ${classElement} ${additionalClass}">
      ...
      <div class="dropdown-menu">
        <div class="d-flex flex-column">
          ...
          <div class="${classNoResults} d-none ms-2">No results found</div>
        </div>
      </div>
      ${clearBtn}
    </div>
    ...
    function updateDom() {
        ...
        const dSelectItems =
            dropdown.getElementsByClassName("dselect-items")[0];
